import pytest
from grpcAPI.process_service import IncludeExclude, ChainedFilter
...
class TestChainedFilter:
    ...
    def test_init_with_filters(self):
        ...
        cf = ChainedFilter(
            includes_excludes=[package_filter, module_filter],
            rule_logic="or"
        )
    ...
    def test_mixed_include_exclude_filters(self):
        ...
        package_filter = IncludeExclude(include=["com.example.*"], exclude=["com.example.test.*"])
    ...
    def test_empty_words_list(self):
        ...
        cf = ChainedFilter(
            includes_excludes=[IncludeExclude(include=["test"])],
            rule_logic="and"
        )
    ...
    def test_rule_logic_options(self, rule_logic: str):
        ...
        cf = ChainedFilter(
            includes_excludes=[IncludeExclude()],
    ...
    def test_real_world_service_filtering_scenario(self):
        ...
        package_filter = IncludeExclude(
            include=["com.myapp.*"], 
            exclude=["com.myapp.test.*", "com.myapp.legacy.*"]
        )
        module_filter = IncludeExclude(
            exclude=["*_deprecated", "temp*"]
        )
        tags_filter = IncludeExclude(
            include=["v2"],
            exclude=["experimental", "alpha"]
        )
