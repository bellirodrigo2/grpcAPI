...
def test_abort_sets_state_and_raises(ctx: ContextMock) -> None:
    with pytest.raises(RuntimeError):
        ctx.abort(1, "Error occurred")
    assert ctx.was_aborted()
    assert ctx.abort_info() == (1, "Error occurred")
    ctx.tracker.abort.assert_called_once_with(1, "Error occurred")


def test_abort_with_status_sets_state_and_raises(ctx: ContextMock) -> None:
    class DummyStatus:
        code = 42
        details = "failure"

    with pytest.raises(RuntimeError):
        ctx.abort_with_status(DummyStatus())
...
def test_add_callback_and_cancel(ctx: ContextMock) -> None:
    called = []
...
def test_not_implemented_methods(ctx: ContextMock) -> None:
    ...
    with pytest.raises(NotImplementedError):
        ctx.send_initial_metadata([("key", "value")])
...
def test_add_multiple_callbacks_and_cancel(ctx: ContextMock) -> None:
    flags = []
...
def test_tracker_methods_after_abort(ctx: ContextMock) -> None:
    with pytest.raises(RuntimeError):
        ctx.abort(10, "aborted!")
    assert not ctx.is_active()
    ctx.tracker.abort.assert_called_once_with(10, "aborted!")
    ctx.tracker.is_active.assert_called()


def test_tracker_methods_after_cancel(ctx: ContextMock) -> None:
    ctx.cancel()
    assert not ctx.is_active()
    ctx.tracker.cancel.assert_called_once()
    ctx.tracker.is_active.assert_called()
...
def test_abort_then_cancel_then_check_tracker(ctx: ContextMock) -> None:
    with pytest.raises(RuntimeError):
        ctx.abort(5, "fatal error")
    ctx.cancel()
    assert not ctx.is_active()
